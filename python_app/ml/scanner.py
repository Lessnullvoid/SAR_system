...
The orchestrator tunes the SDR through amplitude- and phase-sensitive
frequency positions (AM, USB, LSB), dwelling long enough for the ML
detector to collect meaningful data.  FM broadcast (88-108 MHz) is
excluded because it suppresses the signal characteristics relevant to
precursor detection — it remains available for manual comparison.

When an anomaly is detected, the dwell time is extended so the system
...
DEFAULT_SCAN_PLAN: List[ScanPosition] = [
    ...
    # The 2-5 MHz range has strong empirical evidence for precursor
    # effects (carrier drift, skip-distance changes, noise-floor shifts).
    ScanPosition(
        label="HF 80m",
        center_hz=3_500_000,
        antenna="loop_antenna",
    ...
    # ── FM broadcast (88-108 MHz) — EXCLUDED from scanner ───────────
    # FM broadcast suppresses amplitude fluctuations, normalizes signal
    # strength, and discards phase information — unsuitable for detecting
